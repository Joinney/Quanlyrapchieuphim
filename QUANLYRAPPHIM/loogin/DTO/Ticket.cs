...
namespace loogin.DTO
{
    public class Ticket
    {
        ...
        public Ticket(DataRow row)
        {
            ...
            this.Price = row["TienBanVe"].ToString() == "" ? 0 : float.Parse(row["TienBanVe"].ToString());

            // 👇 Gán BookingID từ DataRow
            this.BookingID = row["BookingID"] != DBNull.Value ? row["BookingID"].ToString() : null;
        }
        ...
        public string BookingID { get; set; } // 👈 Thuộc tính mới
    }
}